...
fn build_test_app() -> tauri::App<tauri::test::MockRuntime> {
    let builder = test::mock_builder().manage(Services::new());
...
fn smoke_test_auth_and_task_commands() {
    ...
    assert_eq!(login_response.session.email, "smoke@example.com");
    assert_eq!(login_response.session.name, "Smoke Test");
    ...
    assert_eq!(login_error.code, "auth.invalidCredentials");
    ...
    let list_response = test::get_ipc_response(&webview, invoke_request("list_tasks", json!({})))
    ...
    let empty_list = test::get_ipc_response(&webview, invoke_request("list_tasks", json!({})))
