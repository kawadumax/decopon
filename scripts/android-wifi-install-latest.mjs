...
async function findLatestDebugApk() {
  ...
  console.log(`[android-wifi-install-latest] Selected APK: ${latest.path}`);
  return latest.path;
}
...
async function findConnectedWifiDevice() {
  ...
  if (wifiDevices.length > 1) {
    console.warn(
      `[android-wifi-install-latest] Multiple devices detected. Using ${wifiDevices[0].id}.`,
    );
  }
...
async function installApk(deviceId, apkPath) {
  console.log(`[android-wifi-install-latest] Installing to ${deviceId}.`);
  const timeoutMs =
  ...
  console.log("[android-wifi-install-latest] Installation complete.");
}
...
main().catch((error) => {
  console.error("[android-wifi-install-latest] Failed:", error.message);
  process.exitCode = 1;
});
